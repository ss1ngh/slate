...
export class SlateEngine {
  ...
  private selectionBox: { x: number; y: number; w: number; h: number } | null =
    null;
  ...
  private isStaticDirty: boolean = true;
  ...
  constructor(
    ...
    this.interactiveCtx = interactiveContext;
  ...
  private initDPI() {
    ...
    [this.staticCanvas, this.interactiveCanvas].forEach((canvas) => {
    ...
    this.interactiveCtx.scale(this.dpr, this.dpr);
  }
  ...
  private spawnTextOverlay(
    ...
    const adjustSize = () => {
      ...
      textarea.style.width =
        Math.max(100, textarea.value.length * (15 * this.camera.z)) + "px";
    };
    ...
    const commitText = () => {
      ...
      if (text) {
        ...
        this.staticCtx.font = `24px sans-serif`;
        let maxWidth = 0;
    ...
    this.interactiveCanvas.parentElement?.appendChild(textarea);
  ...
  private handleMouseDown(e: MouseEvent) {
    ...
    //eraser : erase shape under cursor immediately
    ...
    if (this.selectedTool === "select") {
      ...
      if (this.selectedShapes.length === 1) {
        ...
        if (handle) {
          ...
          this.resizeOrigShape = JSON.parse(
            JSON.stringify(this.selectedShapes[0]),
          );
          this.history.push(JSON.parse(JSON.stringify(this.shapes)));
          this.redoStack = [];
          return;
  ...
  private updateCursor(x: number, y: number) {
    ...
    if (this.selectedShapes.length === 1) {
      ...
      const handle = this.hitTestHandle(shape, x, y);
      ...
      if (handle) {
        ...
        this.interactiveCanvas.style.cursor = cursorMap[handle] || "crosshair";
        return;
      }

      const hit = this.getShapeAtPosition(x, y);
      if (hit && hit.id === shape.id) {
        this.interactiveCanvas.style.cursor = "move";
        return;
      }
    ...
    if (this.selectedShapes.length > 1) {
      const hit = this.getShapeAtPosition(x, y);
      if (hit && this.selectedShapes.some((s) => s.id === hit.id)) {
        this.interactiveCanvas.style.cursor = "move";
      } else {
        this.interactiveCanvas.style.cursor = "default";
      }
  ...
  private handleMouseMove(e: MouseEvent) {
    ...
    switch (this.currentShape.type) {
      ...
      case "pencil":
        ...
        this.pathCache.set(
          this.currentShape,
          this.getSvgPathFromStroke(this.currentShape.points),
        );
        break;
    }
  ...
  private handleMouseUp() {
    ...
    if (this.isBoxSelecting && this.selectionBox) {
      ...
      if (this.onSelectionChange) {
        this.onSelectionChange(
          this.selectedShapes.length > 0 ? this.selectedShapes[0]! : null,
        );
      }
      this.render();
      return;
    ...
    if (this.isResizing) {
      ...
      this.saveToLocalStorage();
      this.isStaticDirty = true;
      if (this.onShapeUpdate && this.selectedShapes[0])
  ...
  private renderStatic(): void {
    ...
    ctx.fillRect(
      0,
      0,
      this.staticCanvas.width / this.dpr,
      this.staticCanvas.height / this.dpr,
    );
  ...
  private renderInteractive(): void {
    ...
    // preview shape
    if (this.currentShape) {
      this.draw(this.currentShape, ctx);
    }

    // selection UI
    if (this.selectedTool === "select") {
      for (const shape of this.selectedShapes) {
        this.drawSelectionHighlight(shape, ctx);
      }
    }
  ...
  private drawSelectionHighlight(
    ...
    ctx.lineWidth = 1.5 / pz;
    ctx.setLineDash([5 / pz, 4 / pz]);
  ...
  private draw(shape: Shape, context: CanvasRenderingContext2D): void {
    ...
    const dash =
      shape.strokeStyle === "dashed"
        ? [shape.strokeWidth * 4, shape.strokeWidth * 2.5]
        : shape.strokeStyle === "dotted"
          ? [2, shape.strokeWidth * 2.5]
          : [];
    ...
    switch (shape.type) {
      ...
      case "image":
        ...
        if (!shape.element.complete) return;
  ...
  public bringForward() {
    ...
    const sortedSelected = [...this.selectedShapes].sort(
      (a, b) =>
        this.shapes.findIndex((s) => s.id === b.id) -
        this.shapes.findIndex((s) => s.id === a.id),
    );
  ...
  public ungroupShapes() {
    if (
      this.selectedShapes.length !== 1 ||
      this.selectedShapes[0]?.type !== "group"
    )
      return;
    ...
    const dx =
      group.x -
      Math.min(...group.shapes.map((s) => this.getShapeBounds(s).minX));
    const dy =
      group.y -
      Math.min(...group.shapes.map((s) => this.getShapeBounds(s).minY));
  ...
  private hitTestHandle(shape: Shape, px: number, py: number): string | null {
    ...
    switch (shape.type) {
      ...
      case "circle": {
        bx = shape.x - shape.radius - pad;
        by = shape.y - shape.radius - pad;
        bx2 = shape.x + shape.radius + pad;
        by2 = shape.y + shape.radius + pad;
        break;
      }
      case "pencil": {
        const xs = shape.points.map((p) => p.x),
  ...
  private applyResize(px: number, py: number) {
    const shape = this.selectedShapes[0] as Shape | undefined;
    ...
    if (
      ...
      if (shape.type === "text" && orig.type === "text") {
        ...
        this.staticCtx.font = `${shape.fontSize}px sans-serif`;
        let maxWidth = 0;
        const lines = shape.text.split("\n");
        for (const line of lines) {
          const metrics = this.staticCtx.measureText(line);
    ...
    if (orig.type === "circle" && shape.type === "circle") {
      if (handle === "e" || handle === "w")
        shape.radius = Math.abs(px - orig.x);
      if (handle === "n" || handle === "s")
        shape.radius = Math.abs(py - orig.y);
    }
    ...
    if (orig.type === "pencil" && shape.type === "pencil") {
      const oXs = orig.points.map((p) => p.x),
        oYs = orig.points.map((p) => p.y);
